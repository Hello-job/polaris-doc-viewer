...
const MSGRenderer: DocRenderer = ({ mainState: { currentDocument } }) => {
  ...
  useEffect(() => {
    ...
    const _fd = new MSGReader(currentDocument.fileData as ArrayBuffer).getFileData();
    setFileData(_fd);
  ...
  useEffect(() => {
    ...
    let iframeCont = document.getElementById('msg-body') as HTMLIFrameElement | null;
  ...
  let { recipients, subject, senderEmail, senderName } = fileData as MSGFileData;
        ...
        <ul id="msg-recipient-ul">
          {recipients.map((r, i) => (
            <li key={i} id="msg-recipient-li">
              ...
              {r.hasOwnProperty('email') && <span id="msg-recipient-email"> - {r.email}</span>}
            </li>
          ))}
        </ul>
      </RecipientContainer>
